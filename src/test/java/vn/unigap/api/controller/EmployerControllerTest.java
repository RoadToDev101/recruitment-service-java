...
public class EmployerControllerTest {
    ...
    public void testCreateEmployer() throws Exception {
        // Prepare test data
        EmployerDtoIn employerDtoIn = new EmployerDtoIn("test4@example.com","Test Employer",1L,"Test description");
        EmployerDtoOut mockEmployer = new EmployerDtoOut(1L, employerDtoIn.getEmail(), employerDtoIn.getName(), new JobProvince(employerDtoIn.getProvince(), "Hồ Chí Minh"), employerDtoIn.getDescription());
        ...
        mockMvc.perform(post("/api/v1/employers")
                ...
                .content(asJsonString(employerDtoIn)))
    ...
    public void testUpdateEmployer() throws Exception {
        // Prepare test data
        EmployerUpdateDtoIn employerUpdateDtoIn = new EmployerUpdateDtoIn("Update Test Employer", 1L, "Update description");
        EmployerDtoOut mockEmployer = new EmployerDtoOut(1L, "test4@example.com", employerUpdateDtoIn.getName(), new JobProvince(employerUpdateDtoIn.getProvince(), "Hồ Chí Minh"), employerUpdateDtoIn.getDescription());

        // Mocking service method
        when(employerService.updateEmployer(anyLong(), any(EmployerUpdateDtoIn.class))).thenReturn(mockEmployer);

        // Perform PATCH request and assert results
        mockMvc.perform(patch("/api/v1/employers/1")
                .contentType(MediaType.APPLICATION_JSON)
                .content(asJsonString(employerUpdateDtoIn)))
                ...
                .andExpect(jsonPath("$.object.province.name").value("Hồ Chí Minh"))
                .andExpect(jsonPath("$.object.description").value(employerUpdateDtoIn.getDescription()));
    ...
    public void testGetEmployerById() throws Exception {
        ...
        EmployerDtoOut mockEmployer = new EmployerDtoOut(3093725L, "27f9bad99edfff6434d80a334016d975b3ba1e91@sieu-viet.com", "3shomes", province, "vận hành căn hộ cho thuê");

        // Mocking service method
        when(employerService.getEmployerById(anyLong())).thenReturn(mockEmployer);

        // Perform GET request and assert results
        mockMvc.perform(get("/api/v1/employers/3093725")
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.errorCode").value(0))
                .andExpect(jsonPath("$.statusCode").value(200))
                ...
                .andExpect(jsonPath("$.object.email").value("27f9bad99edfff6434d80a334016d975b3ba1e91@sieu-viet.com"))
    ...
    public void testGetAllEmployers() throws Exception {
        ...
        PageDtoOut<EmployerDtoOut> mockPageDtoOut = new PageDtoOut<EmployerDtoOut>(1, 10, 20L, 2L, Arrays.asList(
            new EmployerDtoOut(1L, "test1@example.com", "Employer 1", new JobProvince(1L, "Hồ Chí Minh"), "Description 1"),
            new EmployerDtoOut(2L, "test2@example.com", "Employer 2", new JobProvince(2L, "Hà Nội"), "Description 2")
        ));
        ...
        // Perform GET request and assert results
        mockMvc.perform(get("/api/v1/employers")
                .contentType(MediaType.APPLICATION_JSON)
                .param("page", String.valueOf(pageDtoIn.getPage()))
                .param("size", String.valueOf(pageDtoIn.getSize())))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.errorCode").value(0))
                .andExpect(jsonPath("$.statusCode").value(200))
    ...
    public void testDeleteEmployerById() throws Exception {
        ...
        mockMvc.perform(delete("/api/v1/employers/{id}", employerId))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.errorCode").value(0))
                .andExpect(jsonPath("$.statusCode").value(200))
